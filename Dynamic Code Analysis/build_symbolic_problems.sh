...
do
	echo ${PROBLEM_FILE}
    java -cp target/aistr.jar nl.tudelft.instrumentation.Main --type=symbolic --file=${PROBLEM_FILE} > instrumented/$(basename ${PROBLEM_FILE})
    javac -cp target/aistr.jar:lib/com.microsoft.z3.jar:. Errors.java instrumented/$(basename ${PROBLEM_FILE})
done