...
public class MyVar {
    ...
    /*
    * Overrides the clone method for MyVar class
    */
    @Override
    public MyVar clone() {
        MyVar aux = new MyVar(this.value);
        aux.type = this.type;
        aux.operator = this.operator;
        aux.str_value = this.str_value;
        aux.int_value = this.int_value;
        aux.left = null;
        aux.right = null;

        if (this.left != null) {
            aux.left = this.left.clone();
        }

        if (this.right != null) {
            aux.right = this.right.clone();
        }
